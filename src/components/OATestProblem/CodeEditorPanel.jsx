...
// export const CodeEditorPanel = ({ boilerplate, programmingLanguages, question}) => {
...
export function CodeEditorPanel({ question, onFinalSubmit }) {
  ...
  const availableLanguages = Object.keys(question?.starter_code[0] || {});
  
  const [selectedLanguage, setSelectedLanguage] = useState(
    availableLanguages[0] || 'javascript' // Default to first available or JS
  );
  const [code, setCode] = useState(() => {
    // Initialize code with starter code for the selected language
    return question.starter_code[0]?.[selectedLanguage] || getDefaultBoilerplate(selectedLanguage);
  });
  ...
  useEffect(() => {
    ...
    setCode(question.starter_code?.[selectedLanguage] || getDefaultBoilerplate(selectedLanguage));
    setOutput('');
    setStatus('');
    setExecutionTime(null);
    setMemoryUsage(null);
  }, [question.question_id, selectedLanguage, question.starter_code, availableLanguages]);

  // Handle language change from dropdown
  const handleLanguageChange = (e) => {
    const newLang = e.target.value;
    if (newLang !== selectedLanguage) {
      if (window.confirm('Changing language will reset your code to the starter template. Are you sure?')) {
  ...
  const runCode = async () => {
    ...
    try {
      ...
      do {
        ...
        resultResponse = await axios.get(`http://localhost:5000/api/code/status/${submissionToken}`, { withCredentials: true });
        submissionStatus = resultResponse.data.status?.description; // Judge0 status

      } while (submissionStatus === 'In Queue' || submissionStatus === 'Processing');
      ...
      if (result.status?.id === 3) { // Status ID 3 is Accepted (Judge0 convention)
        // Further logic needed if multiple test cases for visible cases
        if (result.expected_output === result.stdout.trim()) { // Simple check for single case
             setStatus('Passed (Visible Test Cases)');
        } else {
             setStatus('Failed (Wrong Answer)');
        }
      } else if (result.status?.id >= 5 && result.status?.id <= 12) { // Runtime Error, Compile Error, Time Limit Exceeded etc.
      ...
      setOutput('Error during execution: ' + (error.response?.data?.message || error.message));
  ...
  const submitCode = async () => {
    ...
    if (!code || !selectedLanguage) {
        toast.error('Code and language must be selected before final submission.');
    ...
    try {
        ...
        const response = await axios.post('http://localhost:5000/api/test/submit-answer', {
        ...
        toast.error('An error occurred during final submission. Please try again.');
    } finally {
        setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 text-gray-100">
      ...
      <div className="bg-gray-800 !p-4 border-t border-gray-700 overflow-y-auto max-h-[30%]">
        <h3 className="text-md font-semibold !mb-2">Output <span className={`text-sm !ml-2 ${status.includes('Passed') ? 'text-green-400' : status.includes('Failed') || status.includes('Error') ? 'text-red-400' : 'text-gray-400'}`}>({status})</span></h3>
        {executionTime && memoryUsage && (
            <p className="text-xs text-gray-400 !mb-1">Time: {executionTime} | Memory: {memoryUsage}</p>
        )}
